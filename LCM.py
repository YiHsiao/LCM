...
def is_notebook_run():
    msg = check_output("ps aux | grep python",shell = True).decode()
    for i in msg.split('\n'):
        if 'notebook' in i:
            return True
    return False
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        if(GPIO.input(btn_pin) == 0):
            break
        if(GPIO.input(18) == 0 and (not is_notebook_run())):
            print("--" * 20)
            system("/home/pi/bin/run_ipython_ntoebook.sh")
        if(is_notebook_run()):
            myLCD.move(2,1)
            myLCD.write_msg("ipy notebook run")
